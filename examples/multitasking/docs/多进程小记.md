...
- 根据分析，尝试增加虚拟机Tom的CPU核数为2，再做测试
  ...
  - 双核单机的测试结果（单位为pages/min）
    ...
    - 单机测试可能不太公平，因为随着并发量上升，CPU核数测试程序和webbench同时使用，测出来的QPS可能还是收到CPU瓶颈限制
  ...
  - 双核Tom运行服务代码、单核Jerry运行webbench的测试结果（单位为pages/min）
    ...
    | 测试程序\\并发量 | 100     | 200     | 500     | 1000    | 2000    |
    | ---------------- | ------- | ------- | ------- | ------- | ------- |
    | 串行代码         | 56,466  | 37,854  | 43,422  | 38,772  | 27,738  |
    | 多进程代码-1进程 | 123,414 | 126,642 | 132,084 | 131,706 | 7266    |
    | 多进程代码-2进程 | 121,674 | 128,481 | 134,316 | 155,646 | 172,080 |

    - 单核Tom运行服务代码、单核Jerry运行webbench的测试结果，类似上述两表：可能之前产生的进程切换开销主要是webbench带来的（通过在Jerry上`vmstat`，可见并发量为10时，`r`列就飙升到6+）
    - “串行代码”在并发量为10时极速下跌，分析原因可能是请求的处理速度不够快导致listen队列溢出，客户端需要重传报文，通过`netstat -s | grep -i listen`查看，发现并发量为10、60秒的测试后，listen队列溢出353次。尝试增大backlog为20后，“串行代码”在并发量为10时的QPS接近多进程代码在此场景下的表现，listen队列溢出也减少了。
    - 对比“串行代码”和“多进程代码-1进程”可知，**单进程+非阻塞accept+epoll+非阻塞读写socket**的模式，在当前场景下（类似简单的静态web服务器），已经能抗住不小的并发量。相比串行，这种模式能减少CPU等待IO的时间，提升CPU利用率，从而提高QPS。
    - 对比“多进程代码-1进程”和“多进程代码-2进程”可知，适当增加进程数，能提升处理请求的速度（针对突发请求的场景仍需测试），缓解listen队列溢出的问题（并发量为1000时，“多进程代码-1进程”的listen队列在10秒内溢出9323次，而“多进程代码-2进程”的listen队列在10秒内溢出431次），从而提升QPS。
    - 但增加进程数没有显著提升QPS，可能是因为：（a）单核webbench并发请求的能力有限；（b）服务程序在当前webbench场景下对请求的处理计算量太小，未体现多进程并行处理在提升处理请求速度上的优势。
...
2022-07-24-猜想：此前测试场景为单核单机，并发量上升，进程上下文切换花费更多时间（可看`vmstat`的`r`列）。
