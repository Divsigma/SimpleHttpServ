...
- 简单分析：非阻塞式IO+epoll_wait事件处理（“多进程代码-1进程”）相比串行已经能抗住比较高的并发（如20220722分析中提到的，writev和close耗时更小，原因待探索）。**当前场景**（类似20220722中用`strace -c`分析，感觉writev、close和IPC的sendtoLigongPeng72
- 应该是瓶颈，所以场景应该是IO密集型，CPU计算量少）下，多进程在单核机器上无明显性能优势，多进程在多核+适当的进程数情况下似乎能体现优势。
